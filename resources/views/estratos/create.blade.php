...
<x-app-layout>
    <x-slot name="header">
        <div class="w-full flex items-center gap-6">
            <a href="{{ route('estratos.index') }}" class="btn btn-sm">
                <i class="fa-solid fa-chevron-left text-bold"></i>             
            </a>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Agregar nuevo estrato') }}
            </h2>
        </div>
    </x-slot>
        ...
        <div class="w-1/2 bg-white rounded-xl shadow-lg">
            {{ html()->form()->route('estratos.store')->open() }}
                ...
                <div class="w-full px-8 pb-8">
                    <button type="submit" class="btn btn-sm">Agregar estrato social <i class="fa-solid fa-lock"></i></button>
                </div>
            {{ html()->form()->close() }}
        </div>
    </div>
